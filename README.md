...
- **Who** question: the anser of the quesiont should be a person or a group of people, but not concepts related to people, e.g. team or race.

- **When** question: concept of time, the duration or the moment.

- **What** question: the answer of the question should be intended to entities, objects except those for **who** and **when** questiont.

- **Affirmation**: A closed question which can be answered either 'yes' or 'no'.

- **Unkown**: any other question not falling in previous categories. 


##Data
The question data are part of questions from the following link:
...
>Run code/preprocessing.py, we could get preprocessed data *data/questions_label* and *data/stats* for statistics of the types of sentences.

##Scripts
4 scripts included:

- code/preprocessing.py: preprocess raw data data/raw/raw_questions_label, output /data/questions_lebel which is used for testing, and /data/stats. Every time questions_label will be shuffled.
- code/solutions.py: 4 solutions to the QTI problem
	1. Random Guess: random assign a label to a test sentence, and get average result of 25% (obviously).
	...
	4. Regular expression + syntactic analysis: we could notice that there is almost no disambiguations for **who** and **when**, and the disambiguations lie in the word *what* and *which*, which could also represent the concept of people and the concept of time. The methods could be described as follows:
	
````
1. Use stanford PCFG parser to parse all senteces;
...
18.             assign who type
19.         assign what type
20. 	assign unkonwn type
21. assign affirmation type	            
````
...
the word *model* and *star* for the most usual sense is object rather than person, and maker vice versa.	
...
This is due to the synset of time, because some time concept has measure for the least common_sent, so other mearsure concept words could be considered as time.

- g:2 p:1 What relative of the racoon is sometimes known as the cat-bear
- g:2 p:1 What predators exist on Antarctica

This is due to the most common sen we use, which is misleading and need help of context. 
...
- Here we decide to model **Unknown** type instead of **Affirmation** type, because we think it is easier and the auxilary word for **Affirmation** could be a lot. But logicaaly we ought to form tha latter first.
...
- If we could get large amount of data, machine learning methods are to be taken using SVM or neural network for multi-classification with the help of context.
